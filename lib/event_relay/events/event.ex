defmodule ER.Events.Event do
  ...
  def new_with_defaults(attrs) do
    ...
    |> put_decoded_data()
    |> put_decoded_data_schema()
    |> put_id_if_empty()
    |> coerce_prev_id()
    |> Map.drop([:data_json, :data_schema_json, :durable])
  end

  defp coerce_prev_id(%{prev_id: prev_id} = attrs) do
    if Flamel.blank?(prev_id), do: Map.put(attrs, :prev_id, nil), else: attrs
  end

  defp put_decoded_data(%{data_json: data_json} = attrs) do
  ...
  defp put_decoded_data(attrs) do
    attrs
  end

  defp put_decoded_data_schema(%{data_schema_json: data_schema_json} = attrs) do
  ...
  defp put_decoded_data_schema(attrs) do
    attrs
  end

  defp put_id_if_empty(attrs) do
    value = attrs[:id]
